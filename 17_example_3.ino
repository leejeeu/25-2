#include <Servo.h>

// Arduino pin assignment

#define PIN_IR    0         // IR sensor at Pin A0
#define PIN_LED   9
#define PIN_SERVO 10

#define _DUTY_MIN 800  // servo full clock-wise position (0 degree)
#define _DUTY_NEU 1700  // servo neutral position (90 degree)
#define _DUTY_MAX 2600  // servo full counter-clockwise position (180 degree)

#define _DIST_MIN  100.0   // minimum distance 100mm
#define _DIST_MAX  250.0   // maximum distance 250mm

#define EMA_ALPHA  0.6      // for EMA Filter

#define LOOP_INTERVAL 20  
// Loop Interval (unit: msec) : 수가 클 수록 업데이트 간격이 길어짐 
//예) 20ms = 50Hz이므로 서보가 초당 50번 갱신 -> 빠른 반응, 노이즈에 민감
//예) 100ms = 10Hz : 느리지만 서보가 부드럽게 움직임. -> 서보 떨림 줄이고 싶으면 50~100으로

Servo myservo;
unsigned long last_loop_time;   // unit: msec

float dist_prev = _DIST_MIN;
float dist_ema = _DIST_MIN;

void setup()
{
  pinMode(PIN_LED, OUTPUT);
  
  myservo.attach(PIN_SERVO); 
  myservo.writeMicroseconds(_DUTY_NEU);
  
  Serial.begin(1000000);    // 1,000,000 bps
  last_loop_time = millis();
}

void loop()
{
  unsigned long time_curr = millis();
  int duty;
  float a_value, dist_raw;

  // wait until next event time
  if (time_curr < (last_loop_time + LOOP_INTERVAL))
    return;
  last_loop_time += LOOP_INTERVAL;

  a_value = analogRead(PIN_IR);
  dist_raw = ((6762.0 / (a_value - 9.0)) - 4.0) * 10.0 - 60.0;

  if(dist_raw<_DIST_MIN || dist_raw>_DIST_MAX){
    dist_raw = dist_prev;
    digitalWrite(PIN_LED, HIGH);
  }else{
    dist_prev = dist_raw;
    digitalWrite(PIN_LED, LOW);
  }
  
   // Put range Filter code here (_DIST_MIN ~ _DIST_MAX)
  // and turn on LED if the distance is in the range 

  dist_ema = EMA_ALPHA*dist_raw + (1-EMA_ALPHA)*dist_ema; 
  // EMA_ALPHA : 새 데이터 반영 비율 
  //1-EMA_ALPHA : 이전 평균 보존 비율 
  // Put EMA filter code here
      
  //duty = map(dist_ema, _DIST_MIN, _DIST_MAX, _DUTY_MIN, _DUTY_MAX);
  
  duty = (int) (_DUTY_MIN + (dist_ema-_DIST_MIN)*(_DUTY_MAX-_DUTY_MIN)/(_DIST_MAX-_DIST_MIN));
  // Put map() equivalent code here
  
  myservo.writeMicroseconds(duty);

  Serial.print("_DUTY_MIN:");  Serial.print(_DUTY_MIN);
  Serial.print("_DIST_MIN:");  Serial.print(_DIST_MIN);
  Serial.print(",IR:");        Serial.print(a_value);
  Serial.print(",dist_raw:");  Serial.print(dist_raw);
  Serial.print(",ema:");       Serial.print(dist_ema);
  Serial.print(",servo:");     Serial.print(duty);
  Serial.print(",_DIST_MAX:"); Serial.print(_DIST_MAX);
  Serial.print(",_DUTY_MAX:"); Serial.print(_DUTY_MAX);
  Serial.println("");
}
